...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			...
			$result = mysqli_query($conn, $sql);
			if ($result) {
				echo "<script>alert('Selamat! Pendaftaran Anda telah selesai.')</script>";
				$username = "";
				$email = "";
				$_POST['password'] = "";
				$_POST['cpassword'] = "";
			} else {
				echo "<script>alert('Oops! Sepertinya ada yang salah.')</script>";
			}
		} else {
			echo "<script>alert('Oops! Email telah digunakan.')</script>";
		}
...
<head>
	...
	<title>Login Form Pendaftaran Turnamen Mobile Legends</title>
</head>
<body>
	<div class="container">
